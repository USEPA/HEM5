...
class AllOuterReceptorsNonCensus(CsvWriter):
    """
    ...
    def __init__(self, targetDir, facilityId, model, plot_df):
        ...
        self.columns = [rec_id, lat, lon, source_id, ems_type, pollutant, conc, aconc, elev, population, overlap]
    ...
    def generateOutputs(self):
        ...
        for isector in range(1, num_sectors+1):
            for iring in range(1, num_rings):
                ...
                box_truth = np.logical_and.reduce((outerblks_m[:, 9] == sector1,
                                                   outerblks_m[:, 10] == sector2,
                                                   outerblks_m[:, 11] == ring1,
                                                   outerblks_m[:, 12] == ring2))
