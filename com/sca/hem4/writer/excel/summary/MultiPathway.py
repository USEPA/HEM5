    ...
    def generateOutputs(self):
        ...
        for facilityId in self.facilityIds:
            ...
            pathway = [self.categoryName, facilityId, maxRiskAndHI_df.iloc[0][rural_urban], 'MIR', 'All HAP',
                       maxIndivRisks_df.iloc[0][fips] + maxIndivRisks_df.iloc[0][block],
...
#                                                  (allouter_df[overlap] == 'N')]
            ...
            for key in octants.keys():
                ...
                if minDistanceRow is not None:
                    centroidPathway = self.getCentroidPathway(key, minDistanceRow[fips], minDistanceRow[block],
                        riskblocks_df, facilityId, maxRiskAndHI_df)

                    pathways.append(centroidPathway)

        pathways_df = pd.DataFrame(pathways, columns=[category, fac_id, rural_urban, octant, centroid, fips_block,
    ...
    def getCentroidPathway(self, octant, fipsValue, blockValue, riskblocks_df, facilityId, maxRiskAndHI_df):
        quartet_df = riskblocks_df[(riskblocks_df[fips] == fipsValue) & (riskblocks_df[block] == blockValue)]
        ...
        maxPathway = [self.categoryName, facilityId, maxRiskAndHI_df.iloc[0][rural_urban], octant, receptorType,
                      fipsValue + blockValue, quartet_df.iloc[0][lat], quartet_df.iloc[0][lon],
