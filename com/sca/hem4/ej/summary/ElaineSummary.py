...
class ElaineSummary():
    ...
    def create_summary(self, workbook, formats, national_values, values):
        ...
        worksheet.merge_range("B10:C10", 'Minority by Percent',  formats['sub_header_3'])
        ...
        worksheet.merge_range("B15:C15", 'Income by Percent',  formats['sub_header_3'])
        worksheet.write(15, 0, 'Below Poverty Level')
        worksheet.write(16, 0, 'Above Poverty Level')

        worksheet.merge_range("B18:C18", 'Education by Percent',  formats['sub_header_3'])
        ...
        worksheet.merge_range("B21:C21", 'Linguistically Isolated by Percent', formats['sub_header_3'])
        worksheet.write(21, 0, 'Linguistically Isolated')

        # Create notes
        worksheet.merge_range("A23:H25", self.get_notes(),  formats['notes'])
    ...
    def append_national_data(self, national_values, worksheet, formats):
        ...
        worksheet.write_number(5, 1, exposure_value, formats['number'])

        # white
        value = float(data[0][1])
        format = formats['percentage'] if value < 0.01 else formats['int_percentage']
        worksheet.write_number(7, 1, DataModel.round_to_sigfig(value, 2), format)

        # non white
        value = 1 - value if exposure_value > 0 else 0
        format = formats['percentage'] if value < 0.01 else formats['int_percentage']
        worksheet.write_number(8, 1, DataModel.round_to_sigfig(value, 2), format)

        # african american
        value = float(data[0][2])
        format = formats['percentage'] if value < 0.01 else formats['int_percentage']
        worksheet.write_number(10, 1, DataModel.round_to_sigfig(value, 2), format)

        # native american
        value = float(data[0][3])
        format = formats['percentage'] if value < 0.01 else formats['int_percentage']
        worksheet.write_number(11, 1, DataModel.round_to_sigfig(value, 2), format)

        # other
        value = float(data[0][4])
        format = formats['percentage'] if value < 0.01 else formats['int_percentage']
        worksheet.write_number(12, 1, DataModel.round_to_sigfig(value, 2), format)

        # hispanic
        value = float(data[0][5])
        format = formats['percentage'] if value < 0.01 else formats['int_percentage']
        worksheet.write_number(13, 1, DataModel.round_to_sigfig(value, 2), format)

        # below poverty level
        value = float(data[0][11])
        format = formats['percentage'] if value < 0.01 else formats['int_percentage']
        worksheet.write_number(15, 1, DataModel.round_to_sigfig(value, 2), format)

        # above poverty level
        value = 1 - value if exposure_value > 0 else 0
        format = formats['percentage'] if value < 0.01 else formats['int_percentage']
        worksheet.write_number(16, 1, DataModel.round_to_sigfig(value, 2), format)

        # without high school diploma
        value = float(data[0][10])
        format = formats['percentage'] if value < 0.01 else formats['int_percentage']
        worksheet.write_number(18, 1, DataModel.round_to_sigfig(value, 2), format)

        # with high school diploma
        value = 1 - value if exposure_value > 0 else 0
        format = formats['percentage'] if value < 0.01 else formats['int_percentage']
        worksheet.write_number(19, 1, DataModel.round_to_sigfig(value, 2), format)

        # linguistically isolated
        value = float(data[0][13])
        format = formats['percentage'] if value < 0.01 else formats['int_percentage']
        worksheet.write_number(21, 1, DataModel.round_to_sigfig(value, 2), format)
    ...
    def append_data(self, values, worksheet, formats):
        ...
        worksheet.write_number(5, 2, exposure_value, formats['number'])
        ...
        worksheet.write_number(7, 2, value, format)

        # non white
        value = 1 - value if exposure_value > 0 else 0
        worksheet.write_number(8, 2, value, format)

        # african american
        value = float(row_totals[2])
        worksheet.write_number(10, 2, value, format)

        # native american
        value = float(row_totals[3])
        worksheet.write_number(11, 2, value, format)

        # other
        value = float(row_totals[4])
        worksheet.write_number(12, 2, value, format)

        # hispanic
        value = float(row_totals[5])
        worksheet.write_number(13, 2, value, format)

        # below poverty level
        value = float(row_totals[11])
        worksheet.write_number(15, 2, value, format)

        # above poverty level
        value = 1 - value if exposure_value > 0 else 0
        worksheet.write_number(16, 2, value, format)

        # without high school diploma
        value = float(row_totals[10])
        worksheet.write_number(18, 2, value, format)

        # with high school diploma
        value = 1 - value if exposure_value > 0 else 0
        worksheet.write_number(19, 2, value, format)

        # linguistically isolated
        value = float(row_totals[13])
        worksheet.write_number(21, 2, value, format)
