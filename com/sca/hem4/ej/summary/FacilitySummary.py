    ...
    def create_summary(self, workbook, formats, national_values, values, hazard_name=None):
        ...
        if sheets_key in FacilitySummary.sheets.keys():
            if lines_key in FacilitySummary.lines.keys():
                ...
                worksheet = FacilitySummary.sheets[sheets_key] #workbook.get_worksheet_by_name(sheet_name)

                # Get the next line for this sheet
        ...
        worksheet.write(next_line, 1, self.facilityId)
        FacilitySummary.lines[lines_key] = next_line
