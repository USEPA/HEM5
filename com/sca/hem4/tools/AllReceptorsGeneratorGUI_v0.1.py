...
#                targetDir = self.output_dir + "/" + facilityId
                targetDir = os.path.join(self.output_dir, facilityId)
...
layout = [[sg.Text('Chronic Block Summaries Folder', size=(15, 2), pad=((5,0),(5,0))), sg.InputText(), sg.FolderBrowse()],
           [sg.Text('Radius (m)', size=(15,2), pad=((5,0),(15,0))), sg.Input()],
           [sg.Text(size=(40,2),key='-OUTPUT-')],
           [sg.Button('Start'), sg.Button('Abort')]]

window = sg.Window('All Receptor Generator', layout)
...
while True:
    ...
    generator = AllReceptorsGenerator(hem4_output_dir=values[0], radius=int(values[1]))
    generator.generate()
    window['-OUTPUT-'].update("AllReceptors File Generated")
