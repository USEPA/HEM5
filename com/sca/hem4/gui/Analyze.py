...
#        self.b9.grid(row=3, column=0, columnspan=2, sticky="nsew")
    ...
    def runContour(self,  arguments=None):
        try:
            ...
            if appobj != None:
                ...
                appobj.run(debug= False, port=self.contourport)
    ...
    def runEJdash(self,  arguments=None):
        try:
            ...
            if appobj != None:
                ...
                appobj.run(debug= False, port=self.ejport)
    ...
    def browse_button(self, event):
        datatypes = {
            "% of Total Incidence": str, "aconc": np.float64,
            ...
            "Angle (from north)": np.float64, "Block": str,
            "block": str, "BLOCK": str,
            "Block type": str, "can_blk": str,
            "can_latitude": np.float64, "can_longitude": np.float64,
            "can_rcpt_type": str, "can_rsk_interpltd": str,
            "Cancer Risk": np.float64, "Chem, Centroid, or Discrete": str,
            "Conc (µg/m3)": np.float64, "Conc rounded (µg/m3)": np.float64,
            "Conc sci (µg/m3)": np.float64, "CONC_MG": np.float64,
            "devel_blk": str, "devel_hi_interpltd": str,
            "devel_rcpt_type": str, "Developmental HI": np.float64,
            ...
            "Emission type": str, "Emissions (tons/year)": np.float64,
            "Emissions (tpy)": np.float64, "endo_blk": str,
            "endo_hi_interptd": str, "endo_rcpt_type": str,
            ...
            "Facil_id": str, 'Facility_id': str, 'facility_id': str, 'Facid': str, \
            "Facility count": int, "Facility ID": str,
            'FACNAME': str, "FIPS": str,
            "FIPs": str, "fips": str,
            "Fips": str, "Fips + Block": str,
            "Fraction emitted as particulate matter (%)": np.float64, "Hazard Index": np.float64,
            "hema_blk": str, "hema_hi_interptd": str,
            "hema_rcpt_type": str,
            ...
            "Is max conc at any receptor interpolated? (Y/N)": str,
            "Is max populated receptor interpolated? (Y/N)": str,
            "Kidney HI": np.float64,
            "kidney_blk": str,
            "kidney_hi": np.float64,
            "kidney_hi_interptd": str,
            "kidney_rcpt_type": str,
            ...
            "neuro_blk": str,
            "neuro_hi_interpltd": str,
            "neuro_latitude": np.float64,
            "neuro_longitude": np.float64,
            "neuro_rcpt_type": str,
            "Neurological Facilities": int,
            "Neurological HI": np.float64,
            "Neurological Pop": int,
            "neurological_hi": np.float64,
            "Notes": str,
            "Number people exposed to > 1 Developmental HI": int,
            "Number people exposed to > 1 Endocrinological HI": int,
            "Number people exposed to > 1 Hematological HI": int,
            "Number people exposed to > 1 Immunological HI": int,
            "Number people exposed to > 1 Kidney HI": int,
            "Number people exposed to > 1 Liver HI": int,
            "Number people exposed to > 1 Neurological HI": int,
            "Number people exposed to > 1 Ocular HI": int,
            "Number people exposed to > 1 Reproductive HI": int,
            "Number people exposed to > 1 Respiratory HI": int,
            "Number people exposed to > 1 Skeletal HI": int,
            "Number people exposed to > 1 Spleen HI": int,
            "Number people exposed to > 1 Thyroid HI": int,
            "Number people exposed to > 1 Whole Body HI": int,
            "Number people exposed to >= 1 in 1,000 risk": int,
            "Number people exposed to >= 1 in 1,000,000 risk": int,
            "Number people exposed to >= 1 in 10,000 risk": int,
            "Number people exposed to >= 1 in 10,000,000 risk": int,
            "Number people exposed to >= 1 in 100,000 risk": int,
            "Octant or MIR": str,
            "Ocular HI": np.float64,
            "ocular_blk": str,
            "ocular_hi": np.float64,
            "ocular_hi_interptd": str,
            "ocular_rcpt_type": str,
            "Overlap": str,
            "Parameter": str,
            ...
            "repro_blk": str,
            "repro_hi_interptd": str,
            "repro_rcpt_type": str,
            "Reproductive Facilities": int,
            "Reproductive HI": np.float64,
            "Reproductive Pop": int,
            "reproductive_hi": np.float64,
            "resp_blk": str,
            "resp_hi_interpltd": str,
            "resp_latitude": np.float64,
            "resp_longitude": np.float64,
            "resp_rcpt_type": str,
            "Respiratory Facilities": int,
            ...
            "spleen_hi_interptd": str,
            "spleen_rcpt_type": str,
            "Src Cat": str,
            "Thyroid HI": np.float64,
            "thyroid_blk": str,
            "thyroid_hi": np.float64,
            "thyroid_hi_interptd": str,
            "thyroid_rcpt_type": str,
